...
export const MainStages: FC = () => {
  ...
  return (
    <section className="py-11 mx-auto max-w-7xl desktop:px-0 desktop:py-[1.75rem]">
      ...
      <div
        className="desktop:flex desktop:justify-center w-screen desktop:w-[auto] max-md:cursor-grab overflow-x-auto overflow-y-hidden no-scrollbar grid grid-cols-[1fr] justify-[unset]"
        ref={refStages}
      >
...
const MainStagesList: FC = memo(() => {
  return (
    <>
      {stages.map(({ id, title, text }) => (
        <div key={id} className="p-5 md:p-6 rounded-lg bg-brand-light">
